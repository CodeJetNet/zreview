...
| Any participant whose `unique_id` contains `plrt` | various | Test-generated by automated tests | Bulk-cleanable via `WHERE unique_id LIKE '%plrt%'` |

**Rule:** if a TC needs to mutate participant state (balance change, tier change, status change), seed a NEW participant with `plrt` in the unique_id. Never mutate `stan12121212` or any non-`plrt` participant.

### Pre-existing test orgs / programs / tenants

| Entity | Identifier | Where | Purpose |
|---|---|---|---|
| Stan's marketplace tenant | `stan1212test.mydigitalrewards.com` | Marketplace | Stan's personal QA marketplace tenant; safe to test against (NOT prod despite mydigitalrewards domain) |
| Stan's QA campaign redeem | `stanqa.redeem.adrqa.info` | Campaign redeem flow | Per-campaign redeem testing |
| Real QA programs / orgs / SKUs | `<dev — please supply>` | Various | Dev SHALL list specific UUIDs for any program/org/SKU referenced in TCs; mark `[BLOCKED-DEV-CONFIRM]` if unsure |

**Rule:** if your TC references a program / org / SKU by `<placeholder>`, replace with a concrete UUID OR a `[BLOCKED-DEV-CONFIRM]` marker. Don't ship `<placeholder>` to QA — QA can't guess.

### Fixture file locations (in QA repo)

| Type | Path pattern | Purpose |
|---|---|---|
| CSV upload fixtures | `fixtures/csv/*.csv` (when present) | Batch participant / adjustment / transaction uploads |
| JSON request payload fixtures | `fixtures/api/*.json` (when present) | Repeated API request bodies |
| Visual reference baselines | `fixtures/screenshots/*.png` (UI tickets) | Pixel-diff baselines for visual regression |
| Test data factories | `utils/data/*-factory.ts` | Programmatic test data generation with `plrt` markers baked in |

**Rule:** if dev needs a new fixture, create a PR adding it to QA repo OR provide the JSON/CSV inline in the ticket so QA can save it locally.

---

## Cross-cutting conventions (apply to every TC unless explicitly overridden)

### Email inbox

- Every test email recipient SHALL be `suhrobu+<purpose>@alldigitalrewards.com`. Plus-addressing routes to Stan's Workspace inbox.
- Gmail readback uses Workspace API + filters by subject regex within last N minutes. Helpers: `emailReader.getEmailBySubject(...)`, `emailReader.getPasswordResetUrl(...)`.
- Email subject is the primary filter — keep subjects stable across releases OR explicitly declare the change in the §1a API schema diff block.
- Single-use URLs in emails (password reset, SSO invite) are CONSUMED on first use — generate fresh URLs per test (don't reuse).
- Never assume an email arrives instantly — bounded wait loop (default 30s, max 60s for slow SMTP paths).
- Tests that send real email SHALL be tagged `@sends-real-email` (excluded from default CI runs).
...
- For scheduled / cron behavior: dev SHALL provide a manual-trigger endpoint OR clear "wait until X seconds after enqueue" guidance.

### Locale

- Default: `en_US`. Every TC unless otherwise stated tests against `en_US`.
...
- URL `?lang=` query param controls locale on most QA endpoints (e.g., `?lang=en_US` vs `?lang=es_ES`).
- Catalog API returns localized strings — tests assert on language-agnostic identifiers (SKU, ID) when possible, not on translated text.

### Currency / amount

- API request/response amounts are in CENTS as integers (e.g., `"amount": 100` = $1.00) for most ADR endpoints. Confirm per service — some legacy endpoints use dollars as floats (`"amount": 1.00`). Dev SHALL declare the format in the §1a API schema diff.
- Currency code defaults to USD; non-USD requires explicit `currency` field.
- Tests assert on cents-as-integer unless dev's schema diff declares otherwise.
- Display strings (UI) format with currency symbol + 2 decimals (e.g., `$1.00`) — UI TCs assert on the display string format, not the underlying value.

### Idempotency keys

- Idempotency keys are CLIENT-GENERATED (RA convention) — typically `Idempotency-Key` HTTP header set per request.
- Format: UUID v4 OR client-namespaced string (e.g., `plrt-test-12074-org-create-${Date.now()}`).
- Same key + same body within window → same response (replay-safe).
- Same key + different body → 409 Conflict (mismatch).
- Tests SHALL declare whether they expect idempotent behavior (the §9a Test-Angles row 9 expected result includes the idempotency outcome).
- For tests that intentionally replay: generate the key ONCE, fire request twice, assert second response is identical.
...
- Tests for retry behavior (when ticket touches retry config): use webhook.site custom-response to fail N times then succeed; verify retry attempt count + final success.
- Dev SHALL declare retry intervals in the §1a "What changes after merge" block when ticket changes retry behavior.
